...
import { canGenerateReports, canReadTenant, canWriteTenant, createAssessment, createCustomer, createEvidence, createFinding, createGroup, createReport, createRetest, createUser, deleteAssessment, deleteFinding, deleteReport, getDbBackend, listAssessments, listCustomers, listEvidence, listFindings, listGroups, listReports, listRetests, listUsers, migrateDbBackend, updateAssessment, updateAssessmentStatus, updateCustomer, updateFinding, updateFindingStatus, updateGroup, updateReport, updateReportStatus, verifyUser, type EvidenceWithFile, type GroupInput } from './storage.js'
...
app.put('/api/assessments/:id', requireAuth, async (req, res) => { const user = await currentUser(req); const parsed = assessmentSchema.safeParse({ ...req.body, id: req.params.id }); if (!user) return res.status(401).json({ message: 'Nicht authentifiziert' }); if (!parsed.success) return res.status(400).json({ message: 'Ungültige Assessment-Daten' }); const all = await listAssessments(); const row = all.find((entry) => entry.id === req.params.id); if (!row) return res.status(404).json({ message: 'Assessment nicht gefunden' }); if (!canWriteTenant(user, row.customerId) || !canWriteTenant(user, parsed.data.customerId)) return res.status(403).json({ message: 'Keine Berechtigung für diesen Mandanten' }); res.json(await updateAssessment({ ...row, ...parsed.data })) })
...
app.delete('/api/assessments/:id', requireAuth, async (req, res) => { const user = await currentUser(req); if (!user) return res.status(401).json({ message: 'Nicht authentifiziert' }); const all = await listAssessments(); const row = all.find((entry) => entry.id === req.params.id); if (!row) return res.status(404).json({ message: 'Assessment nicht gefunden' }); if (!canWriteTenant(user, row.customerId)) return res.status(403).json({ message: 'Keine Berechtigung für diesen Mandanten' }); const deleted = await deleteAssessment(row.id); res.json({ ok: deleted }) })
...
app.put('/api/findings/:id', requireAuth, async (req, res) => { const user = await currentUser(req); const parsed = findingSchema.safeParse({ ...req.body, id: req.params.id }); if (!user) return res.status(401).json({ message: 'Nicht authentifiziert' }); if (!parsed.success) return res.status(400).json({ message: 'Ungültige Finding-Daten' }); const all = await listFindings(); const row = all.find((entry) => entry.id === req.params.id); if (!row) return res.status(404).json({ message: 'Finding nicht gefunden' }); if (!canWriteTenant(user, row.customerId) || !canWriteTenant(user, parsed.data.customerId)) return res.status(403).json({ message: 'Keine Berechtigung für diesen Mandanten' }); res.json(await updateFinding({ ...row, ...parsed.data })) })
...
app.delete('/api/findings/:id', requireAuth, async (req, res) => { const user = await currentUser(req); if (!user) return res.status(401).json({ message: 'Nicht authentifiziert' }); const all = await listFindings(); const row = all.find((entry) => entry.id === req.params.id); if (!row) return res.status(404).json({ message: 'Finding nicht gefunden' }); if (!canWriteTenant(user, row.customerId)) return res.status(403).json({ message: 'Keine Berechtigung für diesen Mandanten' }); const deleted = await deleteFinding(row.id); res.json({ ok: deleted }) })
...
app.put('/api/reports/:id', requireAuth, async (req, res) => {
  const user = await currentUser(req)
  const parsed = reportSchema.safeParse({ ...req.body, id: req.params.id })
  if (!user) return res.status(401).json({ message: 'Nicht authentifiziert' })
  if (!parsed.success) return res.status(400).json({ message: 'Ungültige Report-Daten' })
  const existing = (await listReports()).find((entry) => entry.id === req.params.id)
  if (!existing) return res.status(404).json({ message: 'Report nicht gefunden' })
  const allowed = parsed.data.scopeType === 'tenant'
    ? !!parsed.data.customerId && canGenerateReports(user, parsed.data.customerId)
    : !!parsed.data.groupId && (user.role === 'admin' || (await listGroups()).find((group) => group.id === parsed.data.groupId)?.tenantIds.some((tenantId) => user.tenantIds.includes(tenantId)))
  if (!allowed) return res.status(403).json({ message: 'Keine Berechtigung für diesen Report-Kontext' })
  res.json(await updateReport({ ...existing, ...parsed.data }))
})
...
app.delete('/api/reports/:id', requireAuth, async (req, res) => { const user = await currentUser(req); if (!user) return res.status(401).json({ message: 'Nicht authentifiziert' }); const existing = (await listReports()).find((entry) => entry.id === req.params.id); if (!existing) return res.status(404).json({ message: 'Report nicht gefunden' }); const allowed = existing.scopeType === 'tenant' ? !!existing.customerId && canGenerateReports(user, existing.customerId) : !!existing.groupId && (user.role === 'admin' || (await listGroups()).find((group) => group.id === existing.groupId)?.tenantIds.some((tenantId) => user.tenantIds.includes(tenantId))); if (!allowed) return res.status(403).json({ message: 'Keine Berechtigung für diesen Report-Kontext' }); const deleted = await deleteReport(existing.id); res.json({ ok: deleted }) })
