...
const initialEvidence: Evidence[] = [
  {
    id: 'e1',
    findingId: 'VL-2026-0001',
    assessmentId: 'a1',
    type: 'Screenshot',
    title: 'Upload dialog before payload execution',
    content: 'Screenshot dokumentiert den Importdialog und den anschließenden Erfolgspfad.',
  },
  {
    id: 'e2',
    findingId: 'VL-2026-0002',
    assessmentId: 'a1',
    type: 'Request',
    title: 'Stored XSS payload request',
    content: 'POST /comments mit Payload <script>alert(1)</script> gegen Admin-Kommentar-Feld.',
  },
]

const initialRetests: Retest[] = [
  {
    id: 'rt1',
    findingId: 'VL-2026-0003',
    assessmentId: 'a2',
    result: 'Teilweise behoben',
    tester: 'Daniel Schuh',
    notes: 'MFA wurde ergänzt, Passwort-Policy jedoch noch nicht vollständig verschärft.',
  },
]
...
function App() {
  ...
  const [retests, setRetests] = useState<Retest[]>(initialRetests)
    ...
    <div className="layout">
      <aside className="sidebar">
        ...
        <div className="sidebar__card">
          ...
          <ul>
            ...
            <li>{evidence.length} Evidence-Einträge</li>
            <li>{retests.length} Retests</li>
          </ul>
        </div>
      </aside>
      ...
      <main className="main">
        <Routes>
          ...
          <Route
            path="/kunden"
            element={<CustomersPage customers={customers} assessments={assessments} onAddCustomer={(customer) => setCustomers((current) => [customer, ...current])} />}
          />
          <Route
            path="/assessments"
            element={<AssessmentsPage customers={customers} assessments={assessments} onAddAssessment={(assessment) => setAssessments((current) => [assessment, ...current])} onStatusChange={(assessmentId, status) => setAssessments((current) => current.map((item) => (item.id === assessmentId ? { ...item, status } : item)))} />}
          />
          <Route
            path="/findings"
            element={<FindingsPage assessments={assessments} findings={findings} evidence={evidence} retests={retests} onAddFinding={(finding) => setFindings((current) => [finding, ...current])} onStatusChange={(findingId, status) => setFindings((current) => current.map((item) => (item.id === findingId ? { ...item, status } : item)))} onAddEvidence={(entry) => setEvidence((current) => [entry, ...current])} onAddRetest={(entry) => setRetests((current) => [entry, ...current])} />}
          />
          <Route
            path="/reports"
            element={<ReportsPage reports={reports} assessments={assessments} findings={findings} onAddReport={(report) => setReports((current) => [report, ...current])} onStatusChange={(reportId, status) => setReports((current) => current.map((item) => (item.id === reportId ? { ...item, status } : item)))} />}
          />
...
function DashboardPage({ customers, assessments, findings, reports, evidence, retests, stats }: { customers: Customer[]; assessments: Assessment[]; findings: Finding[]; reports: Report[]; evidence: Evidence[]; retests: Retest[]; stats: Array<{ label: string; value: string; tone?: 'critical' | 'high' | 'medium' | 'neutral' }> }) {
  return (
    <>
      ...
      <section className="panel-grid panel-grid--wide">
        <article className="panel">
          <div className="panel__header"><h2>Priorisierte Findings</h2><span>Top-Risiken</span></div>
          <FindingsTable rows={findings.slice(0, 3)} />
        </article>
        <article className="panel">
          <div className="panel__header"><h2>Evidence & Retest</h2><span>Workflow-Stand</span></div>
          <div className="list">
            <div className="list__item"><strong>{evidence.length} Evidence-Einträge</strong><small>Artefakte zu Findings und Testschritten</small></div>
            <div className="list__item"><strong>{retests.length} Retests</strong><small>Nachverfolgung von Behebungsständen</small></div>
            <div className="list__item"><strong>{reports.length} Reports</strong><small>Berichtspfad ist als Workflow vorhanden</small></div>
          </div>
        </article>
      </section>
      <section className="panel-grid">
        <article className="panel">
          <div className="panel__header"><h2>Kunden im Fokus</h2><span>{customers.length} Kunden</span></div>
          <div className="list">
            {customers.map((customer) => {
              const linkedAssessments = assessments.filter((assessment) => assessment.customerId === customer.id).length
              return <div className="list__item" key={customer.id}><strong>{customer.name}</strong><span>{customer.sector}</span><small>{customer.contactEmail} · {linkedAssessments} Assessments</small></div>
            })}
          </div>
        </article>
        <article className="panel">
          <div className="panel__header"><h2>Assessment-Pipeline</h2><span>Live</span></div>
          <div className="list">
            {assessments.map((assessment) => <div className="list__item" key={assessment.id}><strong>{assessment.title}</strong><span>{assessment.type} · {assessment.mode}</span><small>{assessment.status}</small></div>)}
          </div>
        </article>
      </section>
    </>
...
function CustomersPage({ customers, assessments, onAddCustomer }: { customers: Customer[]; assessments: Assessment[]; onAddCustomer: (customer: Customer) => void }) {
...
function ReportsPage({ reports, assessments, findings, onAddReport, onStatusChange }: { reports: Report[]; assessments: Assessment[]; findings: Finding[]; onAddReport: (report: Report) => void; onStatusChange: (reportId: string, status: ReportStatus) => void }) {
  ...
  return (
    <>
      ...
      <section className="panel-grid panel-grid--wide">
        <article className="panel"><div className="panel__header"><h2>Report-Liste</h2><span>{reports.length} Reports</span></div><div className="select-list">{reports.map((report) => <button key={report.id} className={`select-list__item ${selectedReport?.id === report.id ? 'is-selected' : ''}`} onClick={() => setSelectedReportId(report.id)}><strong>{report.title}</strong><span>{report.status}</span><small>{report.summary}</small></button>)}</div></article>
        <article className="panel"><div className="panel__header"><h2>Report-Detail</h2><span>{selectedReport?.title ?? '—'}</span></div>{selectedReport ? <div className="detail-stack"><div className="detail-card"><strong>Assessment</strong><small>{linkedAssessment?.title}</small></div><div className="detail-card"><strong>Summary</strong><small>{selectedReport.summary}</small></div><div className="detail-card"><strong>Findings im Report-Kontext</strong>{reportFindings.length ? reportFindings.map((finding) => <small key={finding.id}>{finding.id} · {finding.title} · {finding.severity}</small>) : <small>Aktuell keine Findings verknüpft.</small>}</div><label><span>Status aktualisieren</span><select value={selectedReport.status} onChange={(event) => onStatusChange(selectedReport.id, event.target.value as ReportStatus)}><option>Draft</option><option>Internes Review</option><option>Freigegeben</option><option>Exportiert</option></select></label></div> : null}</article>
      </section>
      <section className="panel-grid"><article className="panel"><div className="panel__header"><h2>Neuen Report anlegen</h2><span>Workflow</span></div><form className="form-grid" onSubmit={(event) => { event.preventDefault(); const nextReport: Report = { id: `r${Date.now()}`, ...form }; onAddReport(nextReport); setSelectedReportId(nextReport.id); setForm({ assessmentId: assessments[0]?.id ?? '', title: '', status: 'Draft', summary: '' }) }}><label><span>Assessment</span><select value={form.assessmentId} onChange={(event) => setForm({ ...form, assessmentId: event.target.value })}>{assessments.map((assessment) => <option key={assessment.id} value={assessment.id}>{assessment.title}</option>)}</select></label><label><span>Titel</span><input value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} required /></label><label><span>Status</span><select value={form.status} onChange={(event) => setForm({ ...form, status: event.target.value as ReportStatus })}><option>Draft</option><option>Internes Review</option><option>Freigegeben</option><option>Exportiert</option></select></label><label className="form-grid__full"><span>Executive Summary / Kurzbeschreibung</span><textarea value={form.summary} onChange={(event) => setForm({ ...form, summary: event.target.value })} rows={4} required /></label><div className="form-grid__full form-actions"><button type="submit">Report speichern</button></div></form></article></section>
    </>
  )
}

function SettingsPage() {
  const [defaults, setDefaults] = useState({ reportTemplate: 'Standard Executive + Technical Report', defaultSeverityModel: 'CVSS v3.1', defaultLead: 'Daniel Schuh', defaultFindingPrefix: 'VL', defaultRetestLabel: 'Standard-Retest', defaultEvidenceType: 'Screenshot' })
  return (
    <>
      <PageHeader eyebrow="Einstellungen" title="Arbeitsstandards und Default-Werte pflegen" />
      <section className="panel-grid">
        <article className="panel"><div className="panel__header"><h2>Projektstandards</h2><span>Workflow</span></div><form className="form-grid"><label><span>Report-Template</span><input value={defaults.reportTemplate} onChange={(event) => setDefaults({ ...defaults, reportTemplate: event.target.value })} /></label><label><span>Severity-Modell</span><input value={defaults.defaultSeverityModel} onChange={(event) => setDefaults({ ...defaults, defaultSeverityModel: event.target.value })} /></label><label><span>Default Lead Tester</span><input value={defaults.defaultLead} onChange={(event) => setDefaults({ ...defaults, defaultLead: event.target.value })} /></label><label><span>Finding-Präfix</span><input value={defaults.defaultFindingPrefix} onChange={(event) => setDefaults({ ...defaults, defaultFindingPrefix: event.target.value })} /></label><label><span>Retest-Label</span><input value={defaults.defaultRetestLabel} onChange={(event) => setDefaults({ ...defaults, defaultRetestLabel: event.target.value })} /></label><label><span>Default Evidence-Typ</span><input value={defaults.defaultEvidenceType} onChange={(event) => setDefaults({ ...defaults, defaultEvidenceType: event.target.value })} /></label></form></article>
        <article className="panel"><div className="panel__header"><h2>Aktuelle Default-Werte</h2><span>Live</span></div><div className="detail-stack"><div className="detail-card"><strong>Template</strong><small>{defaults.reportTemplate}</small></div><div className="detail-card"><strong>Severity</strong><small>{defaults.defaultSeverityModel}</small></div><div className="detail-card"><strong>Lead</strong><small>{defaults.defaultLead}</small></div><div className="detail-card"><strong>Präfix</strong><small>{defaults.defaultFindingPrefix}</small></div><div className="detail-card"><strong>Retest</strong><small>{defaults.defaultRetestLabel}</small></div><div className="detail-card"><strong>Evidence</strong><small>{defaults.defaultEvidenceType}</small></div></div></article>
      </section>
    </>
  )
}
