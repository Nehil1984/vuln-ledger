...
function App() {
  ...
  const [theme, setTheme] = useState<'dark' | 'light'>(() => {
    const saved = localStorage.getItem('vulnledger-theme')
      ...
      <main className="main">
        <header className="app-toolbar">
          ...
          <div className="toolbar-actions">
            <select className="toolbar-select" defaultValue="Deutsch">
              <option>Deutsch</option>
            </select>
            <button className="secondary-button theme-toggle" onClick={() => setTheme((current) => current === 'dark' ? 'light' : 'dark')}>
              {theme === 'dark' ? '☀' : '☾'}
            </button>
          </div>
        </header>
        ...
        <footer className="app-footer">
          ...
          <a className="app-footer__link" href="https://www.apache.org/licenses/LICENSE-2.0" target="_blank" rel="noreferrer">Lizenz und Copyright</a>
        </footer>
      </main>
    </div>
  )
...
function LoginPage({ error, onLogin }: { error: string; onLogin: (username: string, password: string) => Promise<void> }) {
  ...
  return <div className="auth-shell"><form className="auth-card" onSubmit={async (event) => { event.preventDefault(); setSubmitting(true); try { await onLogin(username, password) } finally { setSubmitting(false) } }}><img src="/vulnledger-logo.png" alt="VulnLedger" className="auth-logo" /><h1>VulnLedger Login</h1><p>Für Pentest-Teams: Scope, Findings, Evidence, Retests und Reports an einem Ort.</p><label><span>Benutzername</span><input value={username} onChange={(event) => setUsername(event.target.value)} required /></label><label><span>Passwort</span><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} required /></label>{error ? <div className="error-box">{error}</div> : null}<button type="submit" disabled={submitting}>{submitting ? 'Anmeldung läuft…' : 'Anmelden'}</button></form></div>
}
...
function AdminPage({ users, dbBackend, backupConfig, backups, onRefresh, onUserCreated, onDbBackendChanged, onBackupConfigChanged, onBackupsChanged }: { users: AdminUser[]; dbBackend: DbBackend; backupConfig: BackupConfig | null; backups: BackupRecord[]; onRefresh: () => Promise<void>; onUserCreated: (row: AdminUser) => void; onDbBackendChanged: (backend: DbBackend) => void; onBackupConfigChanged: (cfg: BackupConfig | null) => void; onBackupsChanged: (rows: BackupRecord[]) => void }) {
