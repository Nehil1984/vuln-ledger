    ...
    <div className="layout">
      <aside className="sidebar">
        ...
        <div className="sidebar__meta">
          ...
          <span>{user.tenantIds.length} Mandanten</span>
      ...
      <main className="main">
        <header className="app-toolbar">
          ...
          <div className="toolbar-actions">
            <button className="secondary-button theme-toggle" onClick={() => setTheme((current) => current === 'dark' ? 'light' : 'dark')}>{theme === 'dark' ? '☀' : '☾'}</button>
          </div>
        </header>
...
function ReportsPage({ user, reports, groups, customers, assessments, findings, tenantNameById, groupNameById, onAddReport, onStatusChange }: { user: SessionUser; reports: Report[]; groups: Group[]; customers: Customer[]; assessments: Assessment[]; findings: Finding[]; tenantNameById: Record<string, string>; groupNameById: Record<string, string>; onAddReport: (report: Report) => Promise<void>; onStatusChange: (reportId: string, status: ReportStatus) => Promise<void> }) {
